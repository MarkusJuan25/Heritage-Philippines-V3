...
import { ClientAssignmentPanel, type ClientAssignmentPanelProps } from './ClientAssignmentPanel';
...
const ASSIGNMENT_URL = '/api/clients/client-1/assignment';
 ...
 * endpoints, unlike EditClientForm's single-endpoint mock chains.
 */
...
const DEFAULT_PROPS: ClientAssignmentPanelProps = {
  clientId: 'client-1',
  role: 'ADMIN_MANAGER',
  currentAssignment: null,
};

function renderPanel(overrides: Partial<ClientAssignmentPanelProps> = {}) {
  return render(<ClientAssignmentPanel {...DEFAULT_PROPS} {...overrides} />);
}
...
describe('ClientAssignmentPanel', () => {
  ...
  describe('Prop synchronization (review correction)', () => {
    ...
    it('does not overwrite a just-confirmed optimistic update with an unchanged (stale) currentAssignment prop', async () => {
      ...
      rerender(<ClientAssignmentPanel {...DEFAULT_PROPS} currentAssignment={null} />);

      expect(assignedConsultantSummary()).toHaveTextContent('Maria Santos');
    });

    it('resets all transient form state when clientId changes, without a remount', async () => {
      mockFetch({ eligible: () => eligibleResponse([]) });
      const user = userEvent.setup();
      const { rerender } = renderPanel({
        clientId: 'client-1',
      ...
      // Leaves an in-progress, unsaved validation error on the FIRST Client.
      await user.click(screen.getByRole('button', { name: 'End assignment' }));
      expect(
        screen.getByText('A reason is required when ending an assignment.'),
      ).toBeInTheDocument();

      // Navigates to a DIFFERENT Client, unassigned there — without this
      // component remounting (same instance, only its props change).
      rerender(
        <ClientAssignmentPanel {...DEFAULT_PROPS} clientId="client-2" currentAssignment={null} />,
      );
  ...
  describe('Mutation response validation (review correction)', () => {
    ...
    it('rejects a PUT response targeting a different Client', async () => {
      mockFetch({
        eligible: () =>
          eligibleResponse([{ id: 'tc-1', name: 'Maria Santos', email: 'maria@example.test' }]),
        mutate: () =>
          jsonResponse(200, {
            assignment: assignmentRecord({ assignedStaffId: 'tc-1', clientId: 'client-99' }),
          }),
      });
      const user = userEvent.setup();
      renderPanel({ currentAssignment: null });
    ...
    it('rejects a DELETE response targeting a different Client', async () => {
      mockFetch({
        eligible: () => eligibleResponse([]),
        mutate: () =>
          jsonResponse(200, {
  ...
  describe('Cross-operation state (review correction)', () => {
  ...
  describe('Eligible-consultant pagination (review correction)', () => {
