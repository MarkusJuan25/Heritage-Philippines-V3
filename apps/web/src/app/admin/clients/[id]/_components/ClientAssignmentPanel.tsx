...
// D-025 §10's Client assignment panel: for ADMIN_MANAGER, interactive
// assign/reassign/end-assignment controls calling the existing, unchanged
// `PUT`/`DELETE /api/clients/[id]/assignment` routes and
...
// kind (D-025 §2 — a Travel Consultant may never assign, reassign, or end
// any Client assignment).
...
// generated Prisma runtime — mirrors EditClientForm.tsx's/
...
// now assigned, `clientId` to confirm the returned row genuinely targets
// this Client rather than a different Client, a Lead, or a Booking (the
// three-way XOR CHECK constraint on StaffAssignment means a real Client
// assignment always carries a non-null `clientId` and null `leadId`/
// `bookingId` — checking `clientId` alone is therefore sufficient to prove
// the row is not a Lead/Booking assignment as well), and `endedAt` to
// distinguish an active row from one that was just ended. Every other
// AssignmentRecord field (assignedByUserId, leadId, bookingId, createdAt,
...
function isAssignmentRecordLike(value: unknown): value is AssignmentRecordLike {
  ...
  return (
    ...
    (candidate.clientId === null || isNonEmptyTrimmedString(candidate.clientId)) &&
 ...
 * valid, still active (`endedAt === null` — `setClientAssignment` never
 * returns an already-ended row), and targets exactly this Client
 * (`clientId === expectedClientId`) — never another Client, a Lead, or a
...
function isSetAssignmentSuccess(
  ...
  return assignment.endedAt === null && assignment.clientId === expectedClientId;
}

type EndAssignmentSuccess = { assignment: AssignmentRecordLike | null };

/**
 * `endClientAssignment` returns `null` only when there was no active
 ...
 * still target exactly this Client. A structurally valid record with
 * `endedAt === null` (claiming to still be active) is exactly as invalid
 * here as one targeting a different Client.
 */
...
export type ClientAssignmentPanelProps = {
  clientId: string;
  role: AppRole;
  currentAssignment: ClientAssignmentSummary;
};

/** A stable primitive key for a `ClientAssignmentSummary`, used only to
...
function assignmentKey(assignment: ClientAssignmentSummary): string {
...
/**
 * Client assignment management (D-025 §2/§10). Submits directly to the
 * existing, unchanged `PUT`/`DELETE /api/clients/[id]/assignment` — no
 ...
 * `'TRAVEL_CONSULTANT'`, the only other role D-025 §2 authorizes to reach
 * this page at all) renders a read-only summary with no button, select, or
 * reason input of any kind — never merely a disabled copy of the Admin
 * controls.
 *
 ...
 * authoritative `staffName` from the Client detail read, and this panel's
 ...
 * flight. A `clientId` change (navigating to a different Client without
 * this component remounting) fully resets every piece of transient state
 * — selection, reasons, errors, success messages, and any pending action —
 * so nothing from a previous Client's in-progress form ever leaks into the
...
export function ClientAssignmentPanel({
  ...
  const [displayedAssignment, setDisplayedAssignment] =
    useState<ClientAssignmentSummary>(currentAssignment);
  ...
  // every piece of transient state describes the PREVIOUS Client and must
  ...
  // differs between two different Clients too).
  if (clientId !== priorClientId) {
    ...
    // Same Client — the authoritative assignment prop itself changed
  ...
  // just the first, so an unmounted panel (or a `clientId` change, which
  // does not itself cancel this Client-independent roster fetch) never
  ...
  async function handleAssignSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (isNoOpSelection) {
      ...
      // (D-025 §10: reassigning to the already-assigned consultant is
    ...
    try {
      const response = await fetch(`/api/clients/${clientId}/assignment`, {
        method: 'PUT',
      ...
      if (response.ok) {
        if (!isSetAssignmentSuccess(body, clientId)) {
          setAssignFormError(GENERIC_ASSIGNMENT_ERROR_MESSAGE);
          return;
        }
        // Never assume success or invent the assignee from the submitted
        // selection alone — only the confirmed, clientId-checked
      ...
      // ROLE, ASSIGNMENT_CONFLICT, CLIENT_NOT_FOUND, and any other
      // controlled failure — each already carries a safe, server-crafted
      // message.
  ...
  async function handleEndSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (response.ok) {
        if (!isEndAssignmentSuccess(body, clientId)) {
          setEndFormError(GENERIC_ASSIGNMENT_ERROR_MESSAGE);
          return;
        }
  ...
  return (
    <div className={styles.assignmentPanel}>
      ...
      {eligibleLoading ? (
        ...
        <form onSubmit={handleAssignSubmit} className={styles.clientForm} noValidate>
          {assignFormError ? (
            <p role="alert" className={styles.formAlert}>
              {assignFormError}
            </p>
      ...
      {isReplacing ? (
        <form onSubmit={handleEndSubmit} className={styles.clientForm} noValidate>
          {endFormError ? (
            <p role="alert" className={styles.formAlert}>
              {endFormError}
            </p>
