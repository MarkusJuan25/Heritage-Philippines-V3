 ...
 * field-by-field"). Mirrors features/leads/http.ts's (and
 * features/bookings/http.ts's, features/assignments/http.ts's)
...
export function clientErrorResponse(error: ClientError): Response {
  return NextResponse.json(
...
/**
 * Runs a Client-management service call and shapes its outcome into a
 * Response: `onSuccess(result)` on success, or the standard `ClientError`
 ...
 * still applies — this function never inspects, wraps, or logs the raw
 * error itself, and never leaks a stack trace, database detail, or
 * submitted Client PII into a response.
 */
