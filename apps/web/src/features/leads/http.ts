 ...
 * field-by-field"). Mirrors features/bookings/http.ts and
 * features/assignments/http.ts's identically-named function.
 */
...
export function leadErrorResponse(error: LeadError): Response {
  return NextResponse.json(
 ...
 * to `parseJsonBody`, mirroring features/bookings/http.ts's `parseQuery`.
 */
...
/**
 * Runs a lead-management service call and shapes its outcome into a
 * Response: `onSuccess(result)` on success, or the standard LeadError
 ...
 * still applies.
 */
export async function runLeadAction<T>(
  action: () => Promise<T>,
