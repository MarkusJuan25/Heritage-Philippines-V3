 ...
 * field-by-field"). Mirrors features/assignments/http.ts and
 * features/staff/http.ts's identically-named function.
 */
...
export function bookingErrorResponse(error: BookingError): Response {
  return NextResponse.json(
 ...
 * to `parseJsonBody`, mirroring features/staff/http.ts's `parseQuery`.
 */
...
/**
 * Runs a booking service call and shapes its outcome into a Response:
 * `onSuccess(result)` on success, or the standard BookingError envelope on a
 * known domain error. Any other error is rethrown so the outer `withRole`
 * wrapper's generic 500 handling (.claude/rules/backend.md's "No secret or
 * sensitive-error exposure") still applies.
 */
export async function runBookingAction<T>(
  action: () => Promise<T>,
