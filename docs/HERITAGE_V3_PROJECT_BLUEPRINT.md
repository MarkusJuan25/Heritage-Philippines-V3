...
- **Currency strategy** — **Direction settled (correction, September 25, 2026):** each Booking selects exactly one currency, with no mixed currencies or exchange rates within a Booking (`docs/HERITAGE_V3_DECISIONS_LOG.md` D-019, Accepted July 20, 2026; D-002's status correction). This no longer blocks Phase 4 payment work. The first-release supported-currency list is accepted in D-056 (September 25, 2026) as `currencies-v1`: PHP only. USD is not approved, and adding any currency requires a dated business decision, a precision check, and tests. The original entry is retained for historical traceability: "proposed: the Philippine peso (PHP) is the single billing currency for the MVP, with the Booking currency field (Section 13.4) fixed to it and multi-currency support deferred. This proposal is **pending explicit stakeholder approval**; it affects Phase 1 schema design and blocks Phase 4 payment work."
