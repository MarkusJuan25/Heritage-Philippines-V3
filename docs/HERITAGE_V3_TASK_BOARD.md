...
**Status:** Active since August 18, 2026. Phase 3 became active because the Phase 2 completion gate is now satisfied per D-033 and the Phase 2 status recorded above. The portal invitation flow's schema (D-034 Stage 2, merged) and its staff-facing repository/service/API layer and Resend delivery adapter (D-034 Section 15 Stage 3: `apps/web/src/features/invitations/`, `apps/web/src/app/api/clients/[id]/invitation/**`, `apps/web/src/app/api/webhooks/resend/route.ts`) are implemented and validated in the local development/test environment. Automated delivery remains disabled by default in every environment (no real Resend account/domain/secret configured). Account activation, the client dashboard, the client-facing proposal-response UI, and the Regional Tours view do not yet exist, and no admin UI yet consumes the Stage 3 invitation routes — this phase's completion gate remains unmet.
