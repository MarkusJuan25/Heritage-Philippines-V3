...
- **The database schema currently has 18 locally applied migrations** (`apps/web/prisma/migrations/`), applied to the local `heritage_v3_dev` and/or disposable `heritage_v3_test` databases only. **Staging and production migration application remain unverified and are not claimed** — no migration has been confirmed applied to either environment.
...
- **Portal-invitation email sending is implemented (Stage 3); private object storage remains unimplemented.** The Resend adapter, invitation repository/service, staff-facing invitation API routes, and the verified `/api/webhooks/resend` webhook handler now exist (`apps/web/src/features/invitations/`, D-034 Section 2(b)/Section 15 Stage 3) — see Section 10 below. Automated sending remains operationally disabled by default (`EMAIL_DELIVERY_ENABLED` still defaults to `"false"`, and no real Resend account/domain/secret has been configured in any environment by this change). Private object storage has no provider selection yet. Section 11 below describes its planned boundary, not implemented behavior.
...
Outbound transactional email (portal invitations, payment reminders, message notifications) is a later Phase 3/4 item (ADR-001: "first needed for portal invitations in Phase 3; extended... in Phase 4"). **Resend is the selected automated transactional-email provider for portal invitations** (`docs/HERITAGE_V3_DECISIONS_LOG.md` D-034 Section 2(b)), with an audited manual-email fallback that works independently of automated-send readiness (D-034 Section 2(c)). **The Resend adapter, invitation send/resend/revoke/manual-confirm service and staff-facing API routes, and the verified `/api/webhooks/resend` handler are now implemented** (D-034 Section 15 Stage 3: `apps/web/src/features/invitations/`, `apps/web/src/app/api/clients/[id]/invitation/**`, `apps/web/src/app/api/webhooks/resend/route.ts`) — provider selection and sending capability are no longer open. Boundaries this implementation follows and future stages must preserve:

- Email provider API keys are server-only secrets, one set per environment, never shared between staging and production.
- Local and staging must run in a sandbox/test mode or send only to an explicit allowlist of test recipients — neither may ever send real notifications to real client addresses. **No real Resend account, API key, or webhook secret has been configured in any environment by Stage 3** — `EMAIL_DELIVERY_ENABLED` still defaults to `"false"` in every environment, and the service layer fails closed on an automated-send request unless it, `RESEND_API_KEY`, and `EMAIL_FROM` are all simultaneously present (D-034 Section 12; `features/invitations/resend-adapter.ts`'s `isAutomatedDeliveryEnabled`).
...
- Sensitive content (documents, full payment details, full private conversation contents) must never be placed in an email body or attachment (blueprint Section 12.3; ADR-001) — this is an application-logic rule that email environment configuration must not be used to work around (e.g., no "just email the file" fallback modes, even in non-production environments). The Stage 3 invitation email body contains only an activation link, nothing else.
- The Resend adapter is confined to the invitation feature's own module (`features/invitations/resend-adapter.ts`, per `.claude/rules/architecture.md`'s feature-ownership rule) so a future provider swap touches one boundary, not call sites scattered across the codebase. Integration uses Resend's Node.js/Next.js API-key path (the installed `resend` npm package, pinned to `6.22.1`), not the Vercel Marketplace integration, since the deployment target remains Hostinger (ADR-001). Webhook signature verification uses the same package's own `resend.webhooks.verify()` (confirmed present in the installed version) — no separate `svix` dependency was needed.
- **Portal-invitation email variables** (`apps/web/src/lib/env.ts`, `apps/web/.env.example`; all optional, no real value ever documented here): `RESEND_API_KEY`, `RESEND_WEBHOOK_SECRET`, `EMAIL_FROM`, `EMAIL_REPLY_TO`, and the explicit fail-closed switch `EMAIL_DELIVERY_ENABLED` (default `"false"`). Automated delivery remains operationally disabled until a Resend account exists, DNS control is confirmed, the Heritage Philippines sending domain is verified, the exact sender address is approved, and these secrets are configured through the real deployment environment (D-034 Section 12) — none of that is claimed or performed by this document or by Stage 3.
...
D-034 Stage 5d (`docs/HERITAGE_V3_DECISIONS_LOG.md` D-037 Section 11) implements PostgreSQL-backed rate limiting for the public activation surface — `apps/web/src/features/activation/rate-limit.ts` and `source.ts`. D-038 Section 3/5 narrows this to the two POST routes only (`/api/activation/continue`, `/api/activation/activate`): the static `/activate` shell (replacing the earlier `/activate/[token]` route) performs no rate-limit check, no `PortalInvitation` lookup, and no other database read of any kind.
...
- The request-source resolver (`source.ts`) is default-deny: no trusted proxy header is configured in any environment under D-037's current authorization, so every source falls back to one shared `"unknown-source"` bucket. D-038 Section 6 closes the GET-side exposure class (path/query/RSC/access-log — structurally impossible once no request ever carries the token) but narrows and keeps open two POST-only deployment gates (D-037 Section 12, as re-recorded by D-038 Section 6): verified Hostinger reverse-proxy header-trust behavior for SOURCE-dimension rate limiting on the two POST routes, and confirmation that Hostinger/proxy request-body logging is disabled for those same two POST endpoints specifically (their JSON bodies still carry the raw token — D-038 does not claim otherwise). Activation must not be exposed in staging or production until both are resolved, independent of this rate-limiting implementation's own completion.
...
## 21. V2 Public-Site Link Boundary

D-052 (`docs/HERITAGE_V3_DECISIONS_LOG.md`, the Client Regional Tours contract, accepted September 21, 2026) plans a read-only Regional Tours page in the client portal whose one job is to link a signed-in client to the tour catalogue of the existing public Heritage Philippines website (V2). This section documents the single configuration value that link needs. **The resolver that validates the value is implemented (D-052 Stage 2); the Regional Tours route that consumes it is a later D-052 stage and is not yet built — until that route exists, the variable has no runtime effect.** No `NEXT_PUBLIC_*` variable is introduced (Section 6's "current state" remains true), and V2's own environment configuration remains out of scope (Section 1).

- `APP_V2_PUBLIC_SITE_URL` (`apps/web/.env.example`; resolver `apps/web/src/features/regional-tours/catalogue-url.ts`) — **optional and server-only in local, staging, and production**, with no default. Named per Section 5 (`APP_` prefix, `_URL` suffix). It holds the public origin of the V2 website the Regional Tours page links to. It is a public URL, not a secret, but it still follows Section 6's "default to server-only" rule, and by contract (D-052 §4) it is read at request time by a Server Component rather than baked into a build.
- **It is deliberately not part of the validated required-environment schema** (`apps/web/src/lib/env.ts`). `getServerEnv()` throws for the whole application when any schema field is invalid, and an optional link value must never be able to fail authentication, database access, or any other server code. A dedicated, pure resolver validates it instead; that resolver imports nothing, performs no I/O, and never throws.
...
- **Unavailable behavior.** A missing, empty, or invalid value never throws and never breaks any other part of the application: the Regional Tours page (a later D-052 stage) is specified to show calm "not available" copy and **no link**. The configured value is never echoed in rendered output, logs, or errors, because a misconfigured value can carry credentials. V3 performs no reachability or availability check against V2.
