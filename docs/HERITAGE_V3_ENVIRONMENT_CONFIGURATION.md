...
D-052 (`docs/HERITAGE_V3_DECISIONS_LOG.md`, the Client Regional Tours contract, accepted September 21, 2026) implements a read-only Regional Tours page in the client portal whose one job is to link a signed-in client to the tour catalogue of the existing public Heritage Philippines website (V2). This section documents the single configuration value that link needs. **The resolver that validates the value was implemented in D-052 Stage 2, the `/client/regional-tours` route that consumes it was implemented in Stage 3 and verified in Stage 4, and the variable now has runtime effect wherever it is configured.** No `NEXT_PUBLIC_*` variable is introduced (Section 6's "current state" remains true), and V2's own environment configuration remains out of scope (Section 1).
