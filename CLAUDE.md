...
Phase 1 — Project Foundation and Phase 2 — Admin Dashboard Core are both complete: authentication, the PostgreSQL schema and migration history, staff account/role/assignment management, Booking backend groundwork, and the full Lead → Client → Proposal → Booking admin-dashboard workflow are implemented, tested, and verified end-to-end in a real PostgreSQL-backed environment (see `docs/HERITAGE_V3_TASK_BOARD.md` and `docs/HERITAGE_V3_DECISIONS_LOG.md` D-033). This repository is currently in **Phase 3 — Client Dashboard Core**. No Phase 3 functionality is implemented yet. `docs/HERITAGE_V3_DECISIONS_LOG.md` D-034 is the accepted contract for Phase 3's first slice — Portal Invitation, Account Activation, and the Client Home / Overview — but D-034 is a documentation-only contract, not implemented code; implementation proceeds only through D-034's separately reviewed stages, under Phase 3's own task board checklist and completion gate.
